...
public class VancouverClockPane extends ClockPane {

	double clockRadius;
	double centerX;
	double centerY;
	
	public VancouverClockPane() {
		if(this.hour>=12) this.setBackground(new Background(new BackgroundFill(Color.rgb(0, 0, 0), CornerRadii.EMPTY, Insets.EMPTY)));
		else this.setBackground(new Background(new BackgroundFill(Color.rgb(250, 250, 250), CornerRadii.EMPTY, Insets.EMPTY)));
	}
	public void setCurrentTime() {
	    // Construct a calendar for the current date and time
	    // Set current hour, minute and second
	    Calendar now = Calendar.getInstance();
	    now.setTimeZone(TimeZone.getTimeZone("America/Vancouver"));
	...
	protected void paintClock() {
		...
		if (this.hour>= 12) circle.setFill(Color.DARKGRAY);
		else circle.setFill(Color.YELLOWGREEN);
		getChildren().clear();
		...
		// Draw Name of Clock
		Text clockName = new Text(centerX-clockRadius*0.32, centerY+clockRadius*0.6, "Vancover");
